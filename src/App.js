...
const App = () => {
  ...
  useEffect( function () {
    async function getFacts() {
      ...
      .order('votesInteresting',{ascending: 
        false})
        .limit(10);
...
const NewFactForm = ({ setFacts, setShowForm }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(text && isValidHttpUrl(source) && category && textLength <= 200){
      setIsUploading(true);
      const {data: newFact, error} = await supabase.from("facts").insert([
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      ...
      />
      <span style={{color: textLength > 200 ? "red" : "blue"}}>
        {200 - textLength}
      </span>
      <input type="text" 
      value={source}
      ...
      <select value={category} onChange={(e) => setCategory(e.target.value)} disabled={isUploading}>
        <option value="">Choose a category:</option>
          {CATEGORIES.map((cat, index) => (
            <option key={index} value={cat.name}>{cat.name.toUpperCase()}</option>
          ))}
      </select>
...
const Fact = ({ fact, setFacts }) => {
  ...
  return (
    <li className="fact">
      ...
      <span className='tag'
      style={{backgroundColor: CATEGORIES.find((category) => category.name === fact.category).color}}
      >
      {fact.category}
      </span>
